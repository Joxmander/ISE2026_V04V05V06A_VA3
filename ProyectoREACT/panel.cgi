...
t   border-color: #F4C842;
t   box-shadow: 0 0 0 3px rgba(244,200,66,0.20);
t }
...
t   color: #1A3942;
t   box-shadow: 0 4px 12px rgba(244,200,66,0.35);
t }
t .btn-primary:hover {
...
t .modo-card .desc {
t   font-size: 0.78em; color: #64748b;
t   margin-top: 4px; line-height: 1.4;
t }
t /* === Tema por modo === */
...
t       <div class="consumo-fill" id="consumoFill"></div>
t     </div>
...
t       &mdash; actualizado cada segundo.
t     </div>
t   </section>
t
...
t               <div class="desc">
t                 Simon Says: repite la secuencia de pads. 3 niveles.
t               </div>
t             </div>
t           </div>
t
...
t               <div class="desc">
t                 Golpea con la fuerza objetivo (1-100). 3 rondas.
t               </div>
t             </div>
t           </div>
t
...
t               <div class="desc">
t                 Pulsa solo si NO rojo y agudo. 3 niveles de 20 s.
t               </div>
t             </div>
t           </div>
t
...
t               <div class="desc">
t                 Mantiene la cadencia. 6 golpes por nivel.
t               </div>
t             </div>
t           </div>
t
...
t               onclick="window.open('records.cgi','_blank')">
...
t </div>
t
t t <div id="webCommand" style="display:none;">
c r 4
t </div>
t
t <script>
...
t     var nombre = inp.value.trim() || 'Invitado';
...
t       var url = 'juego.cgi?modo=' + modo
t               + '&nombre=' + encodeURIComponent(nombre);
...
t     var msg = 'El Nodo B entrara en bajo consumo y'
t             + ' solo podra ser despertado con otro comando.'
t             + '\n\n Continuar?';
...
t     if (/sleep/i.test(texto)) {
t       dot.classList.add('sleep');
t     } else if (/desconectado|caido/i.test(texto)) {
t       dot.classList.add('error');
t     }
t   }
t
t   actualizarBarraConsumo(
t     document.getElementById('consumoVal').textContent
t   );
t   ajustarDot(
t     document.getElementById('statusLabel').textContent
t   );
t
...
t       function get(id) {
t         var el = dom.getElementById(id);
t         return el ? el.innerHTML : '';
t       }
...
t       );
t       
t       // Logica para saltar a la pagina de juego si se pulso el joystick
...
t         var inp_jugador = document.getElementById('nombreJugador');
t         var n_jugador = inp_jugador ? inp_jugador.value.trim() : '';
t         if (!n_jugador) n_jugador = 'Invitado';
t         var dir = 'juego.cgi?modo=' + m_juego + '&nombre=' + encodeURIComponent(n_jugador);
