...
static uint16_t Pads_LeerCanal(uint32_t adc_channel) {
    ...
    /* 1. APAGAR EL ADC: Vital para que acepte el cambio de canal y no se mezclen */
    ...
    /* 2. LECTURA DE LIMPIEZA: Drenamos el condensador fantasma */
    HAL_ADC_Start(&hadc_pads);
    if (HAL_ADC_PollForConversion(&hadc_pads, 1U) == HAL_OK) {
        HAL_ADC_GetValue(&hadc_pads); /* Tiramos este valor a la basura */
    }
    HAL_ADC_Stop(&hadc_pads);

    /* 3. LECTURA REAL: Señal 100% pura y del canal correcto */
    HAL_ADC_Start(&hadc_pads);
    if (HAL_ADC_PollForConversion(&hadc_pads, 2U) == HAL_OK) {
        uint16_t val = (uint16_t)HAL_ADC_GetValue(&hadc_pads);
        HAL_ADC_Stop(&hadc_pads); /* Dejamos el ADC apagado para el siguiente ciclo */
        return val;
    }
...
void Pads_CalibrarSensibilidad(void) {
    ...
    for (uint8_t i = 0U; i < PADS_NUM_CANALES; i++) {
        ...
        printf("\n>>> GOLPEA EL SET %d CON TODA TU FUERZA (5 s) <<<\r\n", i+1);
