...
switch (modo_juego) {
    case 1: /* MODO 1: Memoria */
        if (tick_actual - tiempo_inicio_fase >= 3000U) {
            ...
            if (fase_actual >= 3) {
                jugando = false; 
                Monitor_SetEstado(MODO_PWR_IDLE); 
                // Envia trama: 0x50, Nivel, Puntos, 0
    ...
    case 2: /* MODO 2: Fuerza */
        if (tick_actual - tiempo_inicio_fase >= 2000U) {
            tiempo_inicio_fase = tick_actual;
            fase_actual++;
            nivel_alcanzado = fase_actual; // Faltaba actualizar el nivel
            puntos_actuales += 33; 
    ...
    case 3: /* MODO 3: Inhibición */
        if (tick_actual - tiempo_inicio_fase >= 4000U) {
            ...
            fase_actual++;
            nivel_alcanzado = fase_actual;
            puntos_actuales += 20; // ERROR CORREGIDO: Faltaba sumar puntos
    ...
    case 4: /* MODO 4: Ritmo */
        if (tick_actual - tiempo_inicio_fase >= 1000U) {
            tiempo_inicio_fase = tick_actual;
            fase_actual++;
            nivel_alcanzado = fase_actual / 2; // Sube de nivel cada 2 fases
            if(nivel_alcanzado == 0) nivel_alcanzado = 1;
            puntos_actuales += 10;

            if (fase_actual >= 6) {
                jugando = false; 
                Monitor_SetEstado(MODO_PWR_IDLE);
                // ERROR CORREGIDO: Enviabas un '1' fijo en lugar de nivel_alcanzado
                FibraB_SendFrame(0x50, nivel_alcanzado, puntos_actuales, 0x00); 
            }
        }
        break;
